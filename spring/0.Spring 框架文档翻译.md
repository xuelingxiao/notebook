...
> 你可能想在获取JNDI资源时默认这么做, 这种资源的声明周期是在程序外管理的. 特别的, 确保在操作`DataSource`时总是这么做, 因为在Java EE 程序服务器上已知是由问题的.
...
> 这种内部bean依赖声明的方法仅适用于在`@Configuration`内部定义的`@Bean`. 适用寻常的`@Conponent`类不能声明内部bean依赖.
